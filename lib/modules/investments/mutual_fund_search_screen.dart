...
class MutualFundSearchScreen extends StatefulWidget {
  const MutualFundSearchScreen({Key? key}) : super(key: key);
...
class _MutualFundSearchScreenState extends State<MutualFundSearchScreen> {
  ...
  Future<void> _searchFunds(String query) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _searchResults = data.map((json) => MutualFund.fromJson(json)).toList();
          _isLoading = false;
        });
      } else {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Search Mutual Funds'),
      ),
      body: Column(
        children: [
          Padding(
  ...
  Widget _buildInitialState() {
    ...
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.search, size: 64, color: colorScheme.onSurface.withOpacity(0.4)),
          ...
          Text(
            ...
            style: AppTypography.bodyMedium.copyWith(color: colorScheme.onSurface.withOpacity(0.6)),
          ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: AppTypography.bodySmall.copyWith(color: colorScheme.onSurface.withOpacity(0.6)),
          ),
        ],
      ),
    );
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          subtitle: Text(fund.schemeCode, style: AppTypography.bodySmall.copyWith(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.6))),
          onTap: () {
