...
class TransactionBrainService {
  ...
  final _transactionEventController = StreamController<TransactionEvent>.broadcast();
  Stream<TransactionEvent> get transactionEvents => _transactionEventController.stream;
  
  // Stream controller for knowledge updates
  final _knowledgeEventController = StreamController<KnowledgeEvent>.broadcast();
  Stream<KnowledgeEvent> get knowledgeEvents => _knowledgeEventController.stream;
  ...
  Future<bool> logTransaction({
    ...
    try {
      ...
      if (success) {
        ...
        _transactionEventController.add(TransactionEvent(
  ...
  Future<void> _updateRelatedModules(Transaction transaction) async {
    ...
    if (_subscriptionProvider != null && transaction.metadata?['subscriptionId'] != null) {
      debugPrint('📝 Updating subscription: ${transaction.metadata!['subscriptionId']}');
    }
    ...
    if (_bikeProvider != null && transaction.metadata?['bikeId'] != null) {
      debugPrint('📝 Updating bike expense: ${transaction.metadata!['bikeId']}');
    }
  }
  ...
  Future<bool> autoApproveTransaction(DetectedTransaction detected) async {
    ...
    if (_knowledgeBase.shouldAutoApproveTransaction(
      ...
      debugPrint('🧠 Auto-approved (knowledge): ${detected.merchant} ₹${detected.amount}');
    } else if (!detected.shouldAutoApprove) {
      debugPrint('⚠️ Transaction needs review: ${detected.merchant} (${detected.questionToAsk})');
      return false;
    } else {
      debugPrint('🧠 Auto-approved (confidence): ${detected.merchant} ₹${detected.amount}');
    }
    ...
    final result = await logTransaction(
      amount: detected.amount,
      type: detected.type == 'income' ? TransactionType.income : TransactionType.expense,
      categoryId: category,
      accountId: account,
      date: detected.date,
  ...
  void _learnFromTransaction(DetectedTransaction detected) {
    ...
    if (knowledge == null) {
      ...
      _knowledgeEventController.add(KnowledgeEvent(
        type: KnowledgeEventType.learned,
        merchantId: merchantId,
        timestamp: now,
      ));
      
      debugPrint('🧠 Learned new merchant: ${detected.merchant} → ${detected.detectedCategory}');
    } else {
      ...
      final newAmountRange = knowledge.amountRange != null
          ? AmountRange(
              ...
              max: knowledge.amountRange!.max.clamp(detected.amount, double.infinity),
      ...
      knowledge = knowledge.copyWith(
        ...
        typicalAmount: (knowledge.typicalAmount + detected.amount) / 2,
      ...
      debugPrint('🧠 Updated merchant: ${detected.merchant} (${knowledge.transactionCount}x seen)');
    }
  }
  ...
  Future<Map<String, dynamic>> getFinancialSummary() async {
    ...
    if (_transactionProvider != null) {
      ...
      final monthlyTxns = transactions.where((t) => t.date.isAfter(monthStart)).toList();
    ...
    if (_subscriptionProvider != null) {
      summary['monthlySubscriptions'] = _subscriptionProvider!.subscriptions.fold<double>(
        0.0,
